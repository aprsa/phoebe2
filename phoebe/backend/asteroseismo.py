...
def as_norm_atlp1(l, m, Omega, k):
    ...
    @k: ratio of the horizontal to radial displacement
...
def as_xi_theta(l, m, theta, phi, wt, Omega=0, k=0, pole_tolerance=1e-6):
    r"""
    @l: non-radial degree (the number of longitudinal and latitudinal node lines)
    @m: longitudinal order (the number of longitudinal node lines)
    @theta: colatitude, 0 at the +z pole, pi at the -z pole
    ...
    @k: ratio of the horizontal to radial displacement
    @pole_tolerance: tolerance for the mathematical pole at theta=0
        or pi, to avoid division by zero
    ...
    theta[theta < pole_tolerance] = pole_tolerance
    theta[theta > np.pi-pole_tolerance] = np.pi-pole_tolerance
...
def as_xi_phi(l, m, theta, phi, wt, Omega=0, k=0, pole_tolerance=1e-6):
    r"""
    @l: non-radial degree (the number of longitudinal and latitudinal node lines)
    @m: longitudinal order (the number of longitudinal node lines)
    @theta: colatitude, 0 at the +z pole, pi at the -z pole
