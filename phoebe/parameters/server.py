...
def remoteslurm(server, **kwargs):
    ...
    * `isolate_env` (bool, optional, default=False): Whether to clone the
        conda environment per-job.  Only applicable if `crimpl_env='conda'`.
    ...
    * `install_deps` (bool, optional, default=True): Whether to ensure required
        dependencies are installed in conda on the remote machine (adds some
        overhead)
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [BoolParameter(visible_if='crimpl_env:conda', qualifier='isolate_env', value=kwargs.get('isolate_env', False), advanced=True, description='Whether to clone the conda environment per-job.')]
    ...
    params += [BoolParameter(qualifier='install_deps', value=kwargs.get('install_deps', True), description='Whether to ensure required dependencies are installed in virtual environment on the remote machine (adds some overhead)')]
...
def localthread(server, **kwargs):
    ...
    * `install_deps` (bool, optional, default=True): Whether to ensure required
        dependencies are installed in the virtual environment on the remote
        machine (adds some overhead)
